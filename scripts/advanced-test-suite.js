...
console.log('🧪 ADVANCED TEST SUITE - PFF Quiz Engine');
console.log('==========================================');
...
async function runAdvancedTests() {
  ...
  try {
    ...
    const response = IncidentPlaybook.generateResponse(triggers);
  ...
  try {
    ...
    const cannotDeploy = !AcceptanceGates.canDeploy(invalidBank);
    if (cannotDeploy) {
      results.passed++;
  ...
  try {
    ...
    // Test export gating
    const canExport = PrivacyConsent.canExport(validConsent);
    if (canExport) {
      results.passed++;
  ...
  try {
    ...
    const isMemoryOk = PerformanceEnvelope.checkMemory(memoryUsage.heapUsed);
    if (isMemoryOk) {
      results.passed++;
  ...
  try {
    ...
    const canInitSession = BankFreezePolicy.canInitSession(bankId);
    if (!canInitSession) {
      results.passed++;
  ...
  try {
    ...
    const prodConfig = MultiEnvConfig.getConfig('prod');
  ...
  try {
    ...
    // Test sunset policy
    const sunsetPolicy = DeprecationSunset.getSunsetPolicy(bankId);
  ...
  try {
    ...
    const retrieved = ReplayAuditStore.getAuditRecord('test-replay-123');
  ...
  try {
    ...
    try {
      engine.initSession('test-empty', [], './bank/packaged/bank_package.json');
      results.failed++;
    ...
    try {
      engine.initSession('test-duplicate', ['Control', 'Control'], './bank/packaged/bank_package.json');
      results.failed++;
    ...
    try {
      engine.initSession(null, ['Control'], './bank/packaged/bank_package.json');
      results.failed++;
  ...
  try {
    ...
    for (let i = 0; i < 100; i++) {
      sessionPromises.push(engine.initSession(`perf-test-${i}`, ['Control', 'Pace'], './bank/packaged/bank_package.json'));
    }
  ...
  try {
    const { PFFEngine } = require('../engine/index');
    const engine = new PFFEngine();
    const session = engine.initSession('integrity-test', ['Control', 'Pace'], './bank/packaged/bank_package.json');
  ...
  try {
    const { PFFEngine } = require('../engine/index');
    const engine = new PFFEngine();
    const session = engine.initSession('integration-test', ['Control', 'Pace', 'Boundary'], './bank/packaged/bank_package.json');
